...
declare -A POLICIES
policy_prefix=envoy_filters/red_policies
...
stat[2]=instructions,frontend_retired.l1i_miss,frontend_retired.l2_miss
...
do
	...
	do
		SIZE=$(( 2**$i ))
		echo $SIZE
		if test -f $DIR/$policy/latency_$i; then
			continue
		fi
		#taskset -c 3 wrk --latency -t1 -d360s -R1000 "http://$URL/param?query=demo" > $DIR/${policy}/latency_r_${s}_$fcount &
		taskset -c 3 wrk --latency -t1 -d100s -R5000 "$PROT://$URL/:$SIZE" > $DIR/${policy}/latency_${i} &
		sleep 10
		LG_PID=$(ps -C "wrk" -o pid= | tail -1)
		echo $LG_PID
		for s in "${stat[@]}"
		do
			sudo perf record -C 1 -e $s -t $WTID -g -o $DIR/${policy}/${s}_record-$i -- sleep 10
			#sleep 2
			#taskset -c 3 wrk --latency -t1 -d5s -R100 "http://$URL/param?query=demo" > $DIR/${policy}/latency_r_${s}_$fcount 
			
			sleep 2

			sudo perf stat -C 1 -I 100 -e $s -t $WTID -o $DIR/${policy}/${s}_stat-$i -- sleep 10
			#sleep 2
			#taskset -c 3 wrk --latency -t1 -d5s -R100 "http://$URL/param?query=demo" > $DIR/${policy}/latency_s_${s}_$fcount
			sleep 2
		done
			
		toplev.py -v --no-desc -l1 -I 1000 --thread -p $PID sleep 5 2> $DIR/${policy}/toplev-$i
		sleep 2
